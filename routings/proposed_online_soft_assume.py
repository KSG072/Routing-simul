...
class RoutingTableGenerator:
    ...
    def load_flows(self, totals):
        self.flows.clear()
        # 2. totals와 캐시의 키 목록 비교
        totals_keys = set(totals.keys())

        # 4. flow 계산 및 추가
        for src, dst in tqdm(totals_keys, desc="Calculating flows"):
            w_p = totals[(src, dst)]
    ...
    def _reroute_one_flow(self, fkey, overloaded_edges):
        ...
        original_weights = {}
        for edge_info in overloaded_edges:
            u, v = edge_info[0]
    ...
    def solve(self):
        ...
        while update_list:
            ...
            users = self._flows_using_edge(edge0[0])
            if not users:
                # 해당 엣지를 쓰는 플로우가 없으면 다음 엣지로 (드문 경우)
                print(f"Warning: Overloaded edge {edge0[0]} has no flows using it. Skipping.")
                update_list.pop(0)
                continue

            # 가장 우선순위 높은 flow부터 하나씩 재라우팅 시도
            rerouted_successfully = False
            for user_info_tuple in users:
                f_key = user_info_tuple[0]
                if self._reroute_one_flow(f_key, update_list):
...
if __name__ == "__main__":
    ...
    directory = "routing table(ver1.7.2)_softassume"
    ...
    for generation_rate in GENERATION_RATE_LIST_TEST:
        ...
        for time in range(start_time, total_time, update_duration):
            ...
            # --- totals 전처리 시작 ---
            # 1. real_total에 존재하는 (src, dst) 키만 사용하여 totals를 필터링합니다.
            processed_totals = {key: totals.get(key, 0) for key in real_total.keys()}

            # 2. 필터링된 값들의 합을 구해 재정규화하여, 이들의 합이 1이 되도록 합니다.
            total_weight_sum = sum(processed_totals.values())
            if total_weight_sum > 0:
                for key in processed_totals:
                    processed_totals[key] /= total_weight_sum
            # --- totals 전처리 종료 ---
            ...
            gen.load_flows(processed_totals)
            gen.apply_lambda_hist_to_totals(lambda_hist=avg_lambda_hist)

            inference_totals = processed_totals.copy()
            ...
            #     print(f"{key}: {processed_totals.get(key,0)}")
