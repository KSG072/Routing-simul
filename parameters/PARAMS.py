...
# 지상 전용 Ka 버퍼
SENDING_BUFFER_QUEUE_KA = 5 * 10**6  # bits
SENDING_BUFFER_QUEUE_KA_PACKETS = SENDING_BUFFER_QUEUE_KA // (1500 * 8)

# 대기 버퍼
PUBLIC_WAITING_BUFFER_QUEUE_LASER = 1 * 10**9  # bits
PUBLIC_WAITING_BUFFER_QUEUE_LASER_PACKETS = PUBLIC_WAITING_BUFFER_QUEUE_LASER // (1500 * 8)

PUBLIC_WAITING_BUFFER_QUEUE_KA = 40 * 10**6  # bits
PUBLIC_WAITING_BUFFER_QUEUE_KA_PACKETS = PUBLIC_WAITING_BUFFER_QUEUE_KA // (1500 * 8)

# ISL (Inter-Satellite Link) rates - Laser only
ISL_RATE_LASER = 2.5 * 10**9  # bps

# SGL (Satellite-Ground Link) rates - Ka band only
SGL_KA_DOWNLINK = 1.5 * 10**9  # bps
SGL_KA_UPLINK = 2 * 10**9  # bps

# Packet arrival rates
PACKET_ARRIVAL_RATE_KA = (1 * 10**6, 9 * 10**6)  # bps range (SGL)
PACKET_ARRIVAL_RATE_LASER = (80 * 10**6, 350 * 10**6)  # bps range (ISL)
...
GENERATION_RATE_LASER_MIN = int((PACKET_ARRIVAL_RATE_LASER[0] / PACKET_SIZE_BITS) * 0.001)
GENERATION_RATE_LASER_MAX = int((PACKET_ARRIVAL_RATE_LASER[1] / PACKET_SIZE_BITS) * 0.001)

GENERATION_RATE = random.randint(GENERATION_RATE_LASER_MAX, GENERATION_RATE_LASER_MAX)

GENERATION_RATE_LIST = [33, 30]


PACKET_GEN_INTERVAL = 1

MAX_GATEWAY_HOP_DIFF = 4
...
CONGESTION_PROBABILITY = 0.8  # 혼잡 지역에서 생성될 확률